...
static SDL_GPUGraphicsPipeline* psos[6] = { NULL, NULL, NULL, NULL, NULL, NULL};
...
typedef enum { FOGMODE_EXP, FOGMODE_EXP2, FOGMODE_LINEAR, NUM_FOGMODE } FogMode;
static struct Fog { float color[4], density, start, end; FogMode mode; } fog =
{
...
static bool Lesson16_Init(NeHeContext* restrict ctx)
{
	...
	for (int i = 0; i < 6; ++i)
	{
		static const char* const shaderNames[6] =
		{
			"lesson16_unlit_exp", "lesson16_unlit_exp2", "lesson16_unlit_lin",
			"lesson16_lit_exp",   "lesson16_lit_exp2",   "lesson16_lit_lin"
		};
		const NeHeShaderProgramCreateInfo info = { .vertexUniforms = 1, .fragmentSamplers = 1, .fragmentUniforms = 1 };
		SDL_GPUShader* vertexShader, * fragmentShader;
		if (!NeHe_LoadShaders(ctx, &vertexShader, &fragmentShader, shaderNames[i], &info))
		{
			return false;
		}
		psos[i] = SDL_CreateGPUGraphicsPipeline(ctx->device, &(const SDL_GPUGraphicsPipelineCreateInfo)
		{
		...
		if (!psos[i])
		{
			SDL_LogError(SDL_LOG_CATEGORY_APPLICATION,
				"%s: SDL_CreateGPUGraphicsPipeline: %s", shaderNames[i], SDL_GetError());
			return false;
		}
	}
...
static void Lesson16_Quit(NeHeContext* restrict ctx)
{
	...
	for (int i = SDL_arraysize(psos) - 1; i > 0; --i)
	{
		SDL_ReleaseGPUGraphicsPipeline(ctx->device, psos[i]);
	}
}

static void Lesson16_Resize(NeHeContext* restrict ctx, int width, int height)
{
	(void)ctx;
...
static void Lesson16_Draw(NeHeContext* restrict ctx, SDL_GPUCommandBuffer* restrict cmd,
	SDL_GPUTexture* restrict swapchain, unsigned swapchainW, unsigned swapchainH)
{
	...
	{
		.texture = swapchain,
		.clear_color = { 0.5f, 0.5f, 0.5f, 1.0f },  // Clear to fog colour
	...
	SDL_BindGPUGraphicsPipeline(pass, psos[lighting * 3 + fog.mode]);
	...
	struct { Mtx model, projection; struct Light light; } u = { model, projection, light };
	SDL_PushGPUVertexUniformData(cmd, 0, &u, sizeof(u));
	SDL_PushGPUFragmentUniformData(cmd, 0, &fog, sizeof(struct Fog));
...
static void Lesson16_Key(NeHeContext* ctx, SDL_Keycode key, bool down, bool repeat)
{
	(void)ctx;
	...
	if (down && !repeat)
	{
		...
		case SDLK_F:
			...
			break;
		case SDLK_G:
			fog.mode = (fog.mode + 1) % NUM_FOGMODE;
			break;
		default:
			break;
		}
...
{
	.title = "Chris Aliotta & NeHe's Fog Tutorial",
	.width = 640, .height = 480,
	.createDepthFormat = SDL_GPU_TEXTUREFORMAT_D16_UNORM,
	.init = Lesson16_Init,
	.quit = Lesson16_Quit,
	.resize = Lesson16_Resize,
	.draw = Lesson16_Draw,
	.key = Lesson16_Key
};
